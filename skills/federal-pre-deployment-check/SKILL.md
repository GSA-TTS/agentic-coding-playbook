...
description: >
  Run the federal pre-deployment security checklist against a codebase.
  Classifies each check as automated (agent runs tool), semi-automated (agent reads
  files), or manual (agent asks human). Produces a structured checklist report
  merging automated and manual results. Use before deploying any AI-assisted code.
compatibility: Requires shell access for automated checks (git, grep). Python 3.10+ for report generation.
metadata:
  author: wz-gsa
  version: "0.2.0"
  frameworks: "NIST SP 800-53 Rev 5.2 (SA-11, SA-12, CM-2, SI-10, IA-5, SC-13, AU-2)"
---

# Federal Pre-Deployment Check

This skill executes the pre-deployment security checklist from
...
classification of all 58 items. Summary:

| Type               | Count | How It Works                                  |
| ------------------ | ----- | --------------------------------------------- |
| **Automated**      | 18    | Agent runs a tool, checks exit code or output |
| **Semi-automated** | 22    | Agent reads files or config, reports findings |
...
Run `scripts/run-checks.sh` to execute all automatable checks:

```bash
bash skills/federal-pre-deployment-check/scripts/run-checks.sh [repo-path]
```
...
status and notes for each of the 58 items in the checklist format from
...
- The `run-checks.sh` script is read-only. It does not modify files, install packages, or make network calls.
- All 58 items trace back to NIST SP 800-53 controls via `docs/TRACEABILITY.md` Table 3.
