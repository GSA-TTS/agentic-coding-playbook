...
description: >
  Look up NIST SP 800-53 controls, OWASP LLM/Agentic risks, or security keywords
  to find relevant federal AI playbook across all policy documents. Use when a user
  asks about a specific control (e.g., AC-6, SI-10), an OWASP risk (e.g., LLM01,
  Agentic-05), or a security topic (e.g., "secrets management", "input validation").
compatibility: Requires read access to the repository. No external tools needed.
metadata:
  author: wz-gsa
  version: "0.2.0"
...
This skill navigates the Federal Agentic AI Playbook repository to find relevant
policy documents, checklist items, and remediation playbook for a given security
...
- User needs to trace a checklist failure back to playbook
...
4. **Checklist Item -> Control and playbook** (Table 3, items 1.1-10.6)
...
5. Present: control name, where playbook lives, what the checklist verifies, key requirements
...
3. Note which document sections contain playbook
4. Read the referenced sections
5. Present: risk description, mapped controls, playbook locations, checklist items
...
4. Present: checklist requirement, control mapping, remediation playbook
...
3. If not found, read `INDEX.yaml` to get the current document inventory,
   ...
   - Tier 1 documents (core playbook and controls)
...
by `scripts/generate-index.sh` from document frontmatter, so it is always
up to date.
...
- **Control details:** `docs/SECURITY-CONTROLS.md` — full 800-53 overlay with implementation playbook
- **All other documents:** listed in `INDEX.yaml` under `documents:` with tier classification

## Important Notes

- This is a **read-only** skill. It navigates and presents existing playbook — it does not modify any files.
- All playbook references NIST SP 800-53 Rev 5.2 controls at FIPS Moderate impact level.
...
- Items marked "Out of scope (MVP)" in the traceability matrix are explicitly excluded from current playbook (e.g., multi-agent communication, vector/embedding security).
- When presenting playbook, always cite the specific document path and section number.
