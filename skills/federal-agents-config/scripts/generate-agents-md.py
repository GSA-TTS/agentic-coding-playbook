...
def load_config(config_path: str) -> dict:
    ...
    if size > MAX_CONFIG_SIZE:
        print(
            f"Error: Config file too large ({size} bytes, max {MAX_CONFIG_SIZE})",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if missing:
        print(
            f"Error: Missing required config fields: {', '.join(missing)}",
            file=sys.stderr,
        )
        sys.exit(1)
...
def generate_agents_md(config: dict) -> str:
    ...
    sensitive_str = (
        ...
        if sensitive_types
        else "[Specify sensitive data types]"
    )
    storage = config.get("approved_storage", [])
    storage_str = ", ".join(storage) if storage else "[Specify approved storage]"
    secrets_backend = get_default(
        config, "secrets_backend", "[Secrets management tool]"
    )
    ...
    license_restrictions = config.get(
        "license_restrictions", ["No AGPL", "GPL requires legal review"]
    )
    license_str = (
        ", ".join(license_restrictions) if license_restrictions else "None specified"
    )
...
"""
    ...
    if data_class.upper() in ("PII", "PHI", "CUI"):
        pii_line = (
            f"- **PII handling:** Must use field-level encryption, must mask in logs"
        )
    else:
...
- Follow secure coding practices per federal playbook
...
<!--
  ...
  Source: https://github.com/cloud-gov/agentic-ai-playbook
  Generated: {today}
...
def main() -> None:
    if len(sys.argv) < 2:
        print(
            "Usage: generate-agents-md.py <config.json> [output-path]", file=sys.stderr
        )
        sys.exit(1)
    ...
    if output_path:
        ...
        if resolved.exists():
            print(
                f"Warning: {output_path} already exists. Overwriting.", file=sys.stderr
            )
